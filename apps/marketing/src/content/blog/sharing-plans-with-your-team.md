...
With shuvplan, this is straightforward.

### 1. The plan lands in your browser

When Claude calls `ExitPlanMode`, shuvplan's hook intercepts it. Instead of a terminal prompt asking "Do you want to proceed?", a full review UI opens in your browser. You can read through the plan, see each section rendered as markdown, and start annotating.

### 2. You share the plan

You click **Export → Share → Copy Link**. shuvplan compresses the plan markdown and any annotations you've made into a URL hash fragment. The resulting link looks something like:
...
Your senior clicks **Export → Copy Link** to share their annotated version back. You click **Export → Import Review** and paste their URL. shuvplan merges their annotations into your session, deduplicating any overlapping feedback. Now you see both your notes and theirs, with author labels distinguishing who said what.

### 5. You send combined feedback to Claude

With the merged annotations in front of you, you click **Request Changes**. shuvplan formats the combined feedback — deletions, comments, global comments, quick labels, and "looks good" approvals — into structured markdown and sends it back to Claude Code through the hook system. Claude receives specific, actionable feedback and revises the plan.

## How the hook integration works

shuvplan plugs into Claude Code's `PermissionRequest` hook system. The configuration in `hooks.json` watches for `ExitPlanMode` events:
...
When the hook fires, shuvplan reads the plan content from stdin, starts a local HTTP server, and opens the review UI. The server exposes a `/api/plan` endpoint that the browser fetches, and `/api/approve` or `/api/deny` endpoints that resolve the hook's decision. Approving sends an `allow` decision back to Claude Code. Denying sends a `deny` decision with the formatted annotation feedback as the message.
...
The hash fragment of a URL is never sent to a server in HTTP requests — that's part of the HTTP specification. The share portal at `plan.shuv.dev` is a static page. It serves the UI, then the browser reads and decompresses the hash client-side. The server sees nothing.
...
- **Self-hostable.** If even a static page hosted by someone else isn't acceptable, you can [self-host the portal](/docs/guides/self-hosting/) and point shuvplan at it with `PLANNOTATOR_SHARE_URL`.
...
The sharing round-trip adds a review step without leaving the agent workflow. The junior doesn't need to copy-paste a plan into a Google Doc. The senior doesn't need to context-switch into a different tool. It all happens within the same shuvplan session that the hook opened.

## Try it

Install shuvplan as a [Claude Code plugin](/docs/getting-started/installation/), trigger a plan, and click Export → Share. Send the link to a teammate. See what they think.
